...
export default function SyncAuth({ onSyncComplete, blocking = false }) {
    const [isOpen, setIsOpen] = useState(blocking);
    ...
    useEffect(() => {
        ...
        if (token && savedEmail) {
            ...
            if (blocking) setIsOpen(false); // Close if blocking and we have user
            // Auto sync in background on load
            syncData(token, true);
        } else {
            if (blocking) setIsOpen(true);
        }
    }, [blocking]);
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 ${blocking ? 'bg-bg' : 'bg-black/60 backdrop-blur-sm'}`}>
